...
const EditProducts = () => {
    ...
    const deleteProduct=(id)=>{
      ...
      .catch(()=>{console.log("error in deleting product")})
    }
    const editProduct=(id,index)=>{
        ...
        if(productName.classList.contains("newproduct")){
          ...
          Axios.put("http://localhost:3001/editproduct",{productid:id,productname:productNameVal,productprice:productPriceVal})
          ...
          <div key={index} style={{ color: "black", textDecoration: "none" }} className="eachItem" >
            <div className="eachItemInnerDiv">
              <div className="eachItemImageDiv" id={`eachItemImageDiv${index}`}>
                <img src={`uploads/${eachProduct.productimage}`}   style={{height:"100%",width:"fitContent"}} id={`productImage${index}`} alt="" />
